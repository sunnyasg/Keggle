...
<style type="text/css"> body {padding: 10px 10px 10px 10px;} table,th, td {text-align: center;} </style>


<header>
<h1>Boats Segmentation: Factor Analysis</h1>
=============================================

**Sun Kuk Kim, Edoardo Romagnoli, Matthew Price, Shilpa Anjali, Ariyandat Sunny**

<font face="Verdana" color="Black">


**Factor Analysis techniques** - This technique is often used at the early stages of a data analytics project, for example before running a regression with many independent variables (the raw attributes in that case), in order to summarize information (the variation) in correlated raw attributes using a smaller number of manageable **factors** - which are typically uncorrelated or independent. In the process one decreases the number of raw attributes while **keeping most of the information in the data, in a statistical sense**. 

<h2>Business Decisions</h2>
---------------------------------
<blockquote> <p>
Objective 1:Identify the no: of factors that are relevant to decision making
</p></blockquote>
<blockquote> <p>
Objective 2: How to associate each of the variable to a factor
</p></blockquote>

<h2>The Data</h2>
---------------------------------

The first set of data i.e. the first 29 questions capture the attitudes of the consumer on boating. However, as we progress through our next steps, we will reduce these 29 sets into a fewer but equally if not more relevant factors. 
...
All the columns capture data in a numeric format, hence, we can state that the data is metric. We can proceed to step 2.

#### Step 2: Decide whether to scale or standardize the data

Given that variables can have different ranges/ scales, it is important that we "standardize" them. Standardization ensures that the results are not simply driven by one or many variables that have large values. To standardize the variables, we simply use 
***ScaledValue = [(original value - mean)/ std. deviation]***
...
<br>
Therefore, we can confirm that the dataset is centered at '0' and has a deviation of +/- 1. 

#### Step 3:  Check correlation matrix to see if Factor Analysis makes sense

The correlation matrix helps us identify the raw attributes that are highly correlated. 
...
We can plot the eigenvalues in decreasing order of values to get a scree plot. 
...
The scree plot helps us determine that there are five factors that are relevant to decision making. Therefore, going forward we are going to select the five components with highest values (above the abline).
...
<h3>Interpretation of the factors</h3>
###--Sunny--How would you interpret the selected factors?
###Fill text here
###To do so, look at the above table and sort it by component. Say for example, we will first sort it by Component 1. That gives all the questions that are relevant to the component. Identify commonality/ meta attribute. That becomes "New Component 1". Do the same for other components. 
###--Sunny--
